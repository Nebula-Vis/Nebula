...
const getIntersection = (arrays, key) => {
  // 对象数组的交集
  if (key)
    return _.intersectionBy(arrays, key)
  // 值数组的交集
  else
...
const getAggregationValue = (array, type, key) => {
  ...
  if (key) {
    ...
    else throw `No such aggregation type: ${type}`
  }
  // 值的聚合
  else {
    ...
    else if (type == 'count') return array.length // count不允许有key
    else throw `No such aggregation type: ${type}`
  }
}
...
const getDomainOfArray = (array, x, y) => {
  return [
    [getAggregationValue(array, 'min', x), getAggregationValue(array, 'max', x)],
    [getAggregationValue(array, 'min', y), getAggregationValue(array, 'max', y)]
  ]
}
